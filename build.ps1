...
Invoke-AMFProcess $python ('"' + (Join-Path $PSScriptRoot 'payload\verify_install.py') + '" 4.20') (Join-Path $BuildDirectory 'verify')
& $Compiler ('/DRuntimeDir=' + (Split-Path $python -Parent)) (Join-Path $PSScriptRoot 'AMF.iss')
if ($LASTEXITCODE -ne 0) { throw 'Installer compilation failed.' }
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'dist\AMF-4.20-Setup.exe') -Destination (Join-Path $PSScriptRoot 'Setup.exe') -Force
